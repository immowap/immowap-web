import type { MarketIndicatorIcon } from "@/lib/i18n/market-reports";
import { marketTrendsCardClass } from "@/components/market-trends/MarketTrendsInsightCard";

const INDICATOR_ICON_PATHS: Record<MarketIndicatorIcon, string> = {
  mortgage:
  ...
  inflation:
    "M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941M12 6v12",
  demand:
    "M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z",
};

function IndicatorIcon({ name }: { name: MarketIndicatorIcon }) {
  return (
    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-800/[0.06]">
      <svg
        ...
        aria-hidden="true"
      >
        <path d={INDICATOR_ICON_PATHS[name]} />
      </svg>
    </div>
  );
}
...
export function MarketIndicatorCard({ icon, title, description }: MarketIndicatorCardProps) {
  return (
