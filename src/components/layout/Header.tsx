...
export function Header({ locale, t }: HeaderProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full shrink-0 border-b border-black/[0.06] bg-[#F7F5EF] shadow-[0_1px_3px_rgba(15,61,46,0.06)]">
        <div className="relative mx-auto flex h-20 max-w-7xl items-center justify-between gap-6 px-6 md:px-8">
            ...
            {mainNav.map((item) => {
              ...
              if (item.dropdown === "solutions") {
                return (
                  ...
                  >
                    <NavLink href={href} active={active} hasDropdown>
                      {item.label}
                    </NavLink>
                  </div>
                );
          ...
          <div className="hidden items-center gap-4 lg:flex">
            ...
            <Link
              ...
              className="text-[15px] font-medium text-[#0F3D2E]/85 transition-colors hover:text-[#0F3D2E]"
            >
              {t.nav.packages}
            </Link>
            <Button
          ...
          <button
            type="button"
            className="rounded-xl p-2 text-[#0F3D2E] lg:hidden"
